...
export const registerController = (form) => {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (!emailRegExp.test(email)) {
      errors.push('No has introducido un formato de email válido. por favor, prueba de nuevo.')
    }

    // comprobar que las contraseñas son iguales
    if (password !== passwordConfirm) {
      errors.push('Las contraseñas introducidas no coinciden. por favor, prueba de nuevo.')
    }
  ...
  const handleCreateUser = async (name, email, password, form) => {
    try {
      ...
      const event = new CustomEvent("register-ok", {
        detail: {
          message: 'Tu registro se ha realizado con éxito',
          type: 'success'
        }
      });
      form.dispatchEvent(event)
